...
// removed unused import: exercises_screen.dart
import 'login_screen.dart';
import 'community_screen.dart';
import 'admin_screen.dart';
import 'index.dart' show PronunciationScreen, LearnScreen, EditProfileScreen;
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _checkTeacher() async {
    final teacher = await SupabaseService.isTeacher();
    if (mounted) setState(() => _isTeacher = teacher);
    if (!_isTeacher) _loadCurrentClass();
  }

  Future<void> _checkClassStatus() async {
    if (_isTeacher || _isAdmin) return;
    final cls = await SupabaseService.getStudentClass();
    if (mounted) setState(() => _isInClass = cls != null);
  }

  Future<void> _loadCurrentClass() async {
    if (_isTeacher || _isAdmin) return;
    final cls = await SupabaseService.getStudentClass();
    if (mounted) setState(() => _currentClass = cls);
  }

  Future<void> _loadStreak() async {
    final supaId = SupabaseService.currentUserId;
    if (supaId == null) return;
  ...
  void _showClassSheet() {
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: _currentClass == null
            ? _buildJoinClassContent(ctx)
            : _buildInClassContent(ctx),
      ),
    );
  }

  Widget _buildJoinClassContent(BuildContext ctx) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text('Enter the invite code from your teacher',
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            onPressed: () async {
              ...
              final success = await SupabaseService.joinClassByCode(
  ...
  Widget _buildInClassContent(BuildContext ctx) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
                          ...
                          Stack(
                            ...
                            children: [
                              IconButton(
                                ...
                                tooltip: 'Class',
                                icon: Icon(
                                  _currentClass != null ? Icons.group : Icons.group_outlined,
                                  color: _currentClass != null ? Colors.orange : Colors.white70,
                                  size: 24,
                                ),
                              ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!_isAdmin && !_isTeacher)
                        _buildActionCard(
                          themeProvider,
                          icon: _isInClass ? Icons.group : Icons.group_add,
                          title: _isInClass ? 'My Class' : 'Join a Class',
                          subtitle: _isInClass
                              ? 'View your class and assigned scenarios'
                              : 'Enter an invite code from your teacher',
                          onTap: () => Navigator.pushNamed(context, '/my-class').then((_) {
                            _checkClassStatus();
                          }),
                          isHighlighted: _isInClass,
                        ),
                      if (_isTeacher)
                        _buildActionCard(
                          themeProvider,
